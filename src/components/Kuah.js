...
import "../styles/Kuah.css";

class Kuah extends Component {
    ...
    render() {
        const hasil = this.state.produk.filter(x => x.category === "Mie Kuah");
                    ...
                    <tr key = {item.id}>
                        <td rowSpan = "3">
                            <Link to = {`/deskripsi/${item.id}`}>
                                <img src = {item.img} alt = "Mie Kuah" className = "foto-produk" />
                            </Link>
                        </td>
                        <td>Nama Produk : </td>
                        <td>{item.name}</td>
